...
let primesFlat (n : i64) : []i64 =
  ...
  let (sq_primes, _) =
    loop (sq_primes, len) while len < n do
      ...
      let inc_inds = map(\i -> if i == 1 then 0 else inc_inds[i-1]) (iota (length(inds)))
      ...
      let res_iota = segmented_scan (+) 0 flag_iota tmp
      let res_iota_inc = map(\i -> if i == 1 then 0 else res_iota_inc[i-1]) (iota (length(res_iota)))
