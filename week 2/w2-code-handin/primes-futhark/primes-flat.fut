...
let sgmScanInclusive [n] (flg : [n]bool) (arr : [n]f32) : [n]f32 =
  let flgs_vals = 
    scan ( \ (f1, x1) (f2,x2) -> 
            let f = f1 || f2 in
            if f2 then (f, x2)
...
let primesFlat (n : i64) : []i64 =
  ...
  let (sq_primes, _) =
    loop (sq_primes, len) while len < n do
      ...
      let res_iota_inc = sgmScanInclusive flag_iota tmp
      let res_iota = map (\i -> if i==0 then 0 else res_iota_inc[i-1]) (iota (len(res_iota_inc)))  
